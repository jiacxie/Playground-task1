...
{
__global__ void wmmaAsyncStage3Kernel(const half* __restrict__ A,
                                      ...
                                      size_t K)
{
    ...
    half* smem_warp_stream_ptr =
        &smem[0][0] + warp_id * WMMA_M * 2 * C_SMEM_STRIDE;

    const size_t gmem_idx =
        (block_tile_i + warp_id * 2) * WMMA_M * N + block_tile_j * WMMA_N;
...
#pragma unroll
    ...
    const half* A_warp_ptr = &A[block_tile_i * WMMA_M * K] +
                             BLOCK_ROWS / WARPS_PER_BLOCK * K * warp_id;
    const half* B_warp_ptr = &B[block_tile_j * WMMA_N * K] +
...
#pragma unroll
    for (size_t i = 0; i < A_smem_iters; ++i) {
        ...
        A_lane_ptr =
            (int4*) ((half*) A_lane_ptr + CHUNK_COPY_LINES_PER_WARP * K);
...
#pragma unroll
    for (size_t i = 0; i < B_smem_iters; ++i) {
        ...
        B_lane_ptr =
            (int4*) ((half*) B_lane_ptr + CHUNK_COPY_LINES_PER_WARP * K);
    ...
    A_smem_idx = smem_store_off + BLOCK_ROWS / WARPS_PER_BLOCK * warp_id;
    A_lane_ptr = (int4*) (A_warp_ptr + CHUNK_K * WMMA_K +
...
#pragma unroll
    ...
    B_smem_idx = smem_store_off + B_smem_idx_off +
                 BLOCK_COLS / WARPS_PER_BLOCK * warp_id;
    B_lane_ptr = (int4*) (B_warp_ptr + CHUNK_K * WMMA_K +
                          (lane_id / CHUNK_COPY_LINE_LANES) * K) +
                 (lane_id % CHUNK_COPY_LINE_LANES);
    B_smem_idx += lane_id / CHUNK_COPY_LINE_LANES;
...
#pragma unroll
    ...
    wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half,
                   wmma::row_major>
        A_frag[2][WARP_COL_TILES];
    wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half,
                   wmma::col_major>
        B_frag[2][WARP_ROW_TILES];
...
#pragma unroll
    for (size_t i = 0; i < WARP_COL_TILES; ++i) {
        ...
        wmma::load_matrix_sync(A_frag[reg_store_idx][i], A_tile_ptr,
                               AB_SMEM_STRIDE);
    }
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        ...
        wmma::load_matrix_sync(B_frag[reg_store_idx][j], B_tile_ptr,
                               AB_SMEM_STRIDE);
    }
...
#pragma unroll
            for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
                ...
                wmma::mma_sync(C_frag[i][j_s], A_frag[reg_load_idx][i],
                               B_frag[reg_load_idx][j_s], C_frag[i][j_s]);
            }
        }
        ...
        A_smem_idx = smem_store_off + BLOCK_ROWS / WARPS_PER_BLOCK * warp_id;
        A_lane_ptr = (int4*) (A_warp_ptr + tile_k * WMMA_K +
                              (lane_id / CHUNK_COPY_LINE_LANES) * K) +
                     (lane_id % CHUNK_COPY_LINE_LANES);
        A_smem_idx += lane_id / CHUNK_COPY_LINE_LANES;
...
#pragma unroll
        ...
        B_smem_idx = smem_store_off + B_smem_idx_off +
                     BLOCK_COLS / WARPS_PER_BLOCK * warp_id;
        B_lane_ptr = (int4*) (B_warp_ptr + tile_k * WMMA_K +
                              (lane_id / CHUNK_COPY_LINE_LANES) * K) +
                     (lane_id % CHUNK_COPY_LINE_LANES);
        B_smem_idx += lane_id / CHUNK_COPY_LINE_LANES;
...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            ...
            const half* A_tile_ptr =
                &smem[A_smem_idx][((k_step + 1) % CHUNK_K) * WMMA_K];
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            ...
            const half* B_tile_ptr =
                &smem[B_smem_idx][((k_step + 1) % CHUNK_K) * WMMA_K];
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            ...
            wmma::mma_sync(C_frag[i][j_s], A_frag[reg_store_idx][i],
                           B_frag[reg_store_idx][j_s], C_frag[i][j_s]);
        }
    }
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            half* C_tile_ptr =
                smem_warp_tile_ptr + i * C_SMEM_STRIDE * WMMA_M + j * WMMA_N;

            wmma::store_matrix_sync(C_tile_ptr, C_frag[i][j], C_SMEM_STRIDE,
                                    wmma::mem_row_major);
        }
    }
...
#pragma unroll
    for (size_t i = 0; i < WMMA_M; ++i) {
        *((int4*) (src_gmem_warp_stream_ptr + (i * 2 + lane_id / 16) * N) +
          lane_id % 16) = *((int4*) (smem_warp_stream_ptr +
...
size_t initWmmaAsyncStage3()
{
    ...
    size_t smem_max_size = std::max((BLOCK_ROWS + BLOCK_COLS) * AB_SMEM_STRIDE *
                                        sizeof(half) * K_STAGE,
    ...
    // HGEMM_CHECK_CUDART_ERROR(cudaFuncSetAttribute(
        // wmmaAsyncStage3Kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
        // smem_max_size));
