...
__global__ void matmul_v6(const float* A, const float* B, float* C, int M,
...

    {
        ...
        float4_ref(s_b[0][load_b_smem_k][load_b_smem_n]) = float4_ref(r_load_b[0]);
    }
...
#pragma unroll
        for (int tk = 0; tk < BK; tk++) {
            float4_ref(r_comp_a[0]) = float4_ref(s_a[smem_sel][tk][ty * TM / 2]);
            float4_ref(r_comp_a[4]) = float4_ref(s_a[smem_sel][tk][ty * TM / 2 + BM / 2]);
            float4_ref(r_comp_b[0]) = float4_ref(s_b[smem_sel][tk][tx * TN / 2]);
            float4_ref(r_comp_b[4]) = float4_ref(s_b[smem_sel][tk][tx * TN / 2 + BN / 2]);
...
#pragma unroll
        ...
        float4_ref(s_b[smem_sel_next][load_b_smem_k][load_b_smem_n]) =
            float4_ref(r_load_b[0]);
...
#pragma unroll
    for (int tk = 0; tk < BK; tk++) {
        float4_ref(r_comp_a[0]) = float4_ref(s_a[1][tk][ty * TM / 2]);
        float4_ref(r_comp_a[4]) = float4_ref(s_a[1][tk][ty * TM / 2 + BM / 2]);
        float4_ref(r_comp_b[0]) = float4_ref(s_b[1][tk][tx * TN / 2]);
        float4_ref(r_comp_b[4]) = float4_ref(s_b[1][tk][tx * TN / 2 + BN / 2]);
